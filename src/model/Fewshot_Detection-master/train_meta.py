...
max_epochs = int(math.ceil(cfg.max_epoch * 1. / cfg.repeat)) if cfg.tuning else int(max_epochs)
...
def train(epoch):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        loss.backward()
        # OOM 처리 5줄
        # https://github.com/bingykang/Fewshot_Detection/issues/22
        del output
        del loss
        del mask
        del metax
...
def test(epoch):
    ...
    for batch_idx, (data, target) in enumerate(test_loader):
        ...
        if use_cuda:
            ...
            metax = metax.cuda()
            mask = mask.cuda()
        # out of memory 해결하기 위해 with 코드 추가
        #https://github.com/bingykang/Fewshot_Detection/issues/22
        with torch.no_grad():
